	...
	public partial class Service1 : ServiceBase
	{
		...
		public class CrossAttentionScheduler
		{
			...
			public void ReceiveEnergyFeedback(float energyMetric)
			{
				...
				num = utcNow.ToUnixTimeMilliseconds();
				...
				reward = ComputeReward(energyMetric);
		...
		public long GetLevel(int type, long active_threads_cnt, long current_level)
		{
			...
			{
				long num;
				if (active_threads_cnt <= (long)little_num)
				{
					num = Math.Max((long)Math.Ceiling((double)active_threads_cnt / (double)little_per_group_count) - 1L, 0L);
				}
				else
				{
					num = 3L + active_threads_cnt - (long)little_num;
				}
				long num2 = Math.Min(Math.Max(num, current_level), (long)(perfstatenum - 1));
				long num3 = 3L;
				long num4 = (long)(3 + big_num);
				if (num2 <= num3)
				{
					return num2;
				}
			...
			{
				long num5;
				if (active_threads_cnt <= (long)big_num)
				{
					num5 = Math.Max(active_threads_cnt - 1L, 0L);
				}
				else
				{
					long num6 = Math.Max((long)Math.Ceiling((double)(active_threads_cnt - (long)big_num) / (double)little_per_group_count) - 1L, 0L);
					num5 = Math.Min((long)big_num + num6, (long)(perfstatenum - 1));
				}
				long num7 = Math.Max(num5, current_level);
				long num8 = (long)(big_num - 1);
				long num9 = (long)(3 + big_num);
				if (num7 <= num8)
				{
					return num7;
				}
