...
namespace IntlThrdPerfSchd
{
	public class Service1 : ServiceBase
	{
		public class ProcessBehaviorAnalyzer
		{
			...
			private readonly Dictionary<int, ThreadData> _threadData = new Dictionary<int, ThreadData>();
			...
			public void AddOrUpdateThreadData(int threadId, long instructions)
			{
				lock (_lock)
				{
					...
					if (_threadData.TryGetValue(threadId, out var value))
					{
						value.Instructions = instructions;
						value.LastActiveTicks = DateTime.Now.Ticks;
					}
					else
					{
						_threadData[threadId] = new ThreadData
						{
			...
			public ThreadCoreResult GetThreadCoreType(int threadId)
			{
				lock (_lock)
				{
					ThreadCoreResult result = new ThreadCoreResult
					{
						CoreType = -1,
					...
					int count = _threadData.Count;
					if (count >= 64)
					{
						result.CoreType = 0;
						return result;
					}
					List<long> list = _threadData.Values.Select((ThreadData d) => d.Instructions).ToList();
					if (list.Sum() <= 0)
					{
						result.CoreType = 0;
						return result;
					...
					bool flag;
					flag = IsLoadDifferenceLarge(list, num);
					result.IsLoadDifferenceLarge = flag;
					if (flag)
					{
						int count2 = (int)Math.Ceiling((double)count * 0.3);
						HashSet<int> hashSet = (from kvp in _threadData.OrderByDescending((KeyValuePair<int, ThreadData> kvp) => kvp.Value.Instructions).Take(count2)
							select kvp.Key).ToHashSet();
						result.CoreType = ((hashSet.Contains(threadId) && value.Instructions > 200000) ? 1 : 0);
					}
					else
					{
						result.CoreType = ((num > 200000.0) ? 1 : 0);
					}
					return result;
				}
			}
			...
			public void CleanInactiveThreads()
			{
				lock (_lock)
				{
					...
					long timeout = TimeSpan.FromSeconds(ThreadInactiveTimeoutSeconds).Ticks;
			...
			{
				return GetThreadCoreType(threadId).CoreType;
			}
		}
		...
		public class ThreadScheduleQueue
		{
			...
			private readonly Queue<ScheduleItem> _queue = new Queue<ScheduleItem>();
			...
			public bool Add(int threadId, int scheduleCore, IntPtr affinityMask, int maxQueueSize)
			{
				lock (_lock)
				{
					List<ScheduleItem> list = new List<ScheduleItem>(_queue);
					foreach (ScheduleItem item in list)
					{
						...
						item.ScheduleCore = scheduleCore;
						item.AffinityMask = affinityMask;
						_queue.Clear();
						foreach (ScheduleItem item2 in list)
						{
					...
					_queue.Enqueue(new ScheduleItem
					{
						ThreadId = threadId,
			...
			public bool ContainsThreadId(int threadId)
			{
				lock (_lock)
				{
					foreach (ScheduleItem item in _queue)
					{
						if (item.ThreadId == threadId)
						{
							return true;
		...
		public class CrossAttentionScheduler
		{
			...
			public void Train(SchedulerDataset ds, int epochs, float lr)
			{
				...
				for (int i = 0; i < epochs; i++)
				{
					...
					for (int num5 = 0; num5 < ds.Size; num5++)
					{
						...
						float[] item5 = tuple.p;
						float num6 = (float)(-Math.Log(Math.Max((item3 == 0) ? item5[0] : item5[1], 1E-10f)));
						num2 += num6;
						if (item4 == item3)
						{
							num3++;
			...
			public void TrainOnline(float[] x, int label, float lr = 0.1f)
			{
				...
				for (int j = 0; j < 16; j++)
				{
					float num5 = (num - num2) * _out[j, 0] * 0.01f;
					for (int k = 0; k < 5; k++)
					{
						_emb[k, j] -= lr * num5 * x[k] * 0.01f;
				...
				for (int n = 0; n < 2; n++)
				{
					...
					for (int num7 = 0; num7 < 16; num7++)
					{
						float num8 = array[num7] - _coreK[n, num7];
						_coreK[n, num7] += lr * num6 * num8;
						_coreK[n, num7] = Math.Max(-10f, Math.Min(10f, _coreK[n, num7]));
					}
				}
			}
			...
			private void DistributeReward(float reward)
			{
				...
				for (int i = 0; i < _currentWindow.Count; i++)
				{
					...
					float num3 = (float)Math.Exp(-(float)Math.Max(0L, num - scheduleRecord.Timestamp) / (float)_windowMs);
		...
		public class ThreadPriorityMapper
		{
			...
			{
				PriorityClass key = (PriorityClass)priorityClass;
				ThreadPriorityLevel key2 = (ThreadPriorityLevel)threadPriority;
				int value;
				int value2;
				if (!PriorityClassIndex.TryGetValue(key, out value))
				{
					value = 2;
				}
				if (!ThreadPriorityIndex.TryGetValue(key2, out value2))
				{
					value2 = 3;
				}
		...
		public class ThreadClassifier
		{
			...
			private double GetDimensionValue(ThreadData thread, int dimension)
			{
				...
				{
				case 0:
					return thread.InstructionCount;
				case 1:
		...
		public readonly struct ThreadExecutionItem
		{
			...
			public ThreadExecutionItem(int threadId, long execTime, long lastActiveTicks)
			{
				ThreadId = threadId;
				ExecTime = execTime;
				LastActiveTicks = lastActiveTicks;
			}

			public ThreadExecutionItem Update(long newExecTime, long newTicks)
			{
				return new ThreadExecutionItem(ThreadId, newExecTime, newTicks);
			}
		}
		...
		public class ThreadExecutionRegistry
		{
			...
			private readonly Dictionary<int, ThreadExecutionItem> _data = new Dictionary<int, ThreadExecutionItem>();
			...
			public void AddOrUpdate(int threadId, long execTime)
			{
				...
				lock (_lockObj)
				{
					if (_data.TryGetValue(threadId, out var value))
					{
						_data[threadId] = value.Update(execTime, ticks);
					}
					else
					{
						_data.Add(threadId, new ThreadExecutionItem(threadId, execTime, ticks));
					}
				}
			}
			...
			public void CleanInactive(long expireTicks)
			{
				...
				lock (_lockObj)
				{
					foreach (KeyValuePair<int, ThreadExecutionItem> datum in _data)
					{
						if (ticks - datum.Value.LastActiveTicks > expireTicks)
						{
							if (list == null)
							{
								list = new List<int>();
			...
			public int GetMaxExecTimeThreadId()
			{
				lock (_lockObj)
				{
					...
					foreach (KeyValuePair<int, ThreadExecutionItem> datum in _data)
					{
						if (datum.Value.ExecTime > num)
						{
							num = datum.Value.ExecTime;
							result = datum.Key;
						}
					}
					return result;
			...
			public (int threadId, long maxExecTime) GetMaxExecTimeInfo()
			{
				lock (_lockObj)
				{
					if (_data.Count == 0)
					{
						return (threadId: -1, maxExecTime: -1L);
					}
					long num = long.MinValue;
					int item = -1;
					foreach (KeyValuePair<int, ThreadExecutionItem> datum in _data)
					...
					return (threadId: item, maxExecTime: num);
				}
			}
		...
		public class NeuralNetwork
		{
			...
			private void UpdateWeights(ref int[,] weights, int delta, int[] prevLayer, int layerDepth, int iscomp)
			{
				...
				if (iscomp == 1)
				{
					num = 0.1f * (1f / (float)Math.Pow(2.0, layerDepth));
				}
				else
				{
					num = 1f * (1f / (float)Math.Pow(2.0, layerDepth));
				}
		...
		public class SysInfo
		{
			...
			public SysInfo(int max_gid, int min_gid, uint availaff, uint availaff1)
			{
				...
				total_llcmiss = 0L;
				IsModelSaved = false;
				activeThread_little = 0L;
				activeThread_big = 0L;
				activeCoreCount = 0L;
				Datetime4corecount = DateTime.Now.Ticks;
				updateCoreCount = true;
				little_big_ratio = 0f;
				accIdleTime = 0f;
				accIdleCount = 0L;
				avg_idle_time = 0f;
				threadMigrations = 0f;
				threadMigrations_count = 0L;
				avgthreadMigrations = 0f;
				total_slots = 0L;
			}
		}
		...
		public class ThreadInfo
		{
			...
			{
				PrevCoreAffinity = prevCoreAffinity;
				PrevCoreType = prevCoreType;
				Tid = tid;
				DateTime = datetime;
				IntVal = intval;
		...
		public class ProcessInfo
		{
			...
			public ProcessBehaviorAnalyzer analyzer { get; set; }
			...
			public ProcessInfo(int pid, long datetime, long intval, long count, long runtime, long waittime, long duration, long instruction, int coreType, int lock1, long avg_Inspressure, long instruction_little, long instruction_ratio, int index, int perflvl, long maxinsdatetime, long activethreadstat_datetime)
			{
				...
				exchange_queue = 0L;
				MaxThreadLoad = 0L;
				Is_important_process = false;
				analyzer = new ProcessBehaviorAnalyzer();
			}
		}
		...
		public ThreadScheduleQueue queue = new ThreadScheduleQueue();
		...
		public static (float perfTotal, float ecoTotal) CalcCoreActiveThreadSum(uint perfAffinityMask, uint ecoAffinityMask, float[][] coreFeatures)
		{
			float num = 0f;
			float num2 = 0f;
			for (int i = 0; i < coreFeatures.Length; i++)
			{
				...
				if ((perfAffinityMask & (1 << i)) != 0L)
				{
					num += num3;
				}
				else if ((ecoAffinityMask & (1 << i)) != 0L)
				{
					num2 += num3;
				}
			}
			return (perfTotal: num, ecoTotal: num2);
		}

		private int GetCoreType(int coreIndex)
		{
			uint item = (uint)(1 << coreIndex);
			bool flag = bigSmtIndices.Count > 0;
			int num = (flag ? 2 : 1);
			if (bigPhysicalIndices.Contains(item))
			{
				return 0;
			}
			if (flag && bigSmtIndices.Contains(item))
			{
				return 1;
			}
			int num2 = littleIndices.IndexOf(item);
			if (num2 >= 0)
			{
				return num + num2 / 4;
			}
			if (exlittleIndices.Contains(item))
			{
				return num + (littleIndices.Count + 3) / 4;
			}
			return -1;
		}
		...
		public int Intval2Limit(int oldthread, long intval, long utility, long nonstore_store_ratio, ref long usr_ratio_avg, ref long ins_big, int currentprocessor, long usr_ratio, ref long max_ins, ref long usr_ratio1, long br_sys, ref long tag, uint affinity, ref long reset_count, ref long usr_ratio_little, ref long prod_cons_ratio, ref long switch1, ref long lock_data, ref long residence_p1, ref long residence_p)
		{
			...
			if (num == 1)
			{
				if (((int)((uint)(1 << currentprocessor) & affinitymask_little) > 0) & (tag == 2L))
				{
					if (reset_count == 0L)
					{
			...
			else
			{
				if (num == 2)
				{
					if (((int)((uint)(1 << currentprocessor) & affinitymask_big) > 0) & (tag == 6L))
					{
						if (reset_count == 0L)
						{
		...
		public long GetLevel(int type, long active_threads_cnt, long current_level)
		{
			...
			if (type == 3)
			{
				if (active_threads_cnt <= little_num)
				{
					val = Math.Max((long)Math.Ceiling((double)active_threads_cnt / (double)little_per_group_count) - 1, 0L);
				}
				else
				{
					val = 3 + active_threads_cnt - little_num;
				}
				num = Math.Min(Math.Max(val, current_level), perfstatenum - 1);
				num2 = 3L;
				num3 = 3 + big_num;
				if (num <= num2)
				{
		...
		public static bool Show(string title, string message, int style = 0, int timeout = 60, bool wait = true)
		{
			...
			int pResponse;
			return WTSSendMessage(IntPtr.Zero, sessionId, title, title.Length * 2, message, message.Length * 2, style, timeout, out pResponse, wait);
		}
		...
		protected override void OnStart(string[] args)
		{
			...
			for (int num4 = 0; num4 < Convert.ToInt32(NumberOfLogicalProcessors); num4++)
			{
				coretype[num4] = GetCoreType(num4);
			}
			...
			for (int num5 = 0; num5 < maxLP; num5++)
			{
				for (int num6 = 0; num6 <= num5; num6++)
				{
					array6[num5] |= list[num6];
				}
				Perfgroup[num5] = new GroupInfo(0, 0L, 0L, list[num5], 0L, 0L, 0L, 0L, 0L, array6[num5], DateTime.Now.Ticks, 0L);
			}
			uint[] array7 = new uint[list2.Count + 2];
			for (int num7 = 0; num7 < maxLP; num7++)
			{
				for (int num8 = 0; num8 <= num7; num8++)
				{
					array7[num7] |= list2[num8];
				}
				Effgroup[num7] = new GroupInfo(0, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, array7[num7], DateTime.Now.Ticks, 0L);
			}
			big_total = Convert.ToInt32(NumberOfLogicalProcessors) - little_num;
			...
			{
				UIntPtr threadAffinityMask = (UIntPtr)(ulong)(1 << num9);
				uint num10 = 0u;
				uint num11 = 0u;
				myOls.RdmsrTx(1908u, ref num10, ref num11, threadAffinityMask);
				myOls.WrmsrTx(1908u, (num10 & 0xFFFFFF) | 0x80000000u, num11, threadAffinityMask);
				if ((int)((uint)(1 << num9) & affinitymask_little) > 0)
				{
					UIntPtr threadAffinityMask2 = (UIntPtr)(ulong)Math.Pow(2.0, num9);
					myOls.WrmsrTx(390u, 4391025u, 0u, threadAffinityMask2);
					myOls.WrmsrTx(391u, 4391027u, 0u, threadAffinityMask2);
					myOls.WrmsrTx(392u, 4391028u, 0u, threadAffinityMask2);
					myOls.WrmsrTx(393u, 4391618u, 0u, threadAffinityMask2);
				...
				if ((int)((uint)(1 << num9) & (affinitymask_big_phyx | affinitymask_big_smt)) > 0)
				{
					UIntPtr threadAffinityMask3 = (UIntPtr)(ulong)Math.Pow(2.0, num9);
					myOls.WrmsrTx(390u, 4392100u, 0u, threadAffinityMask3);
					myOls.WrmsrTx(391u, 4391588u, 0u, threadAffinityMask3);
					myOls.WrmsrTx(392u, 4391332u, 0u, threadAffinityMask3);
					myOls.WrmsrTx(393u, 4391618u, 0u, threadAffinityMask3);
			...
			long ticks = DateTime.Now.Ticks;
			for (int num12 = 0; num12 < Convert.ToInt32(NumberOfLogicalProcessors); num12++)
			{
				UpdateNode(1, ref wait_core[num12], num12, 0);
				UpdateNode(1, ref max_ipc_queue[num12], -1, -1);
				UpdateNode(1, ref max_util_queue[num12], -1, -1);
				coreinfo[num12] = new CoreInfo(num12, ticks, 0L, 0L, 0L, 0L, 0L, 0L, (uint)(1 << num12), (uint)(1 << num12), ticks, ticks);
				exclude[num12] = 0L;
				exclude_b[num12] = 0L;
				exclude_all[num12] = 0L;
				last_duration[num12] = 0L;
				now_duration[num12] = 0L;
				avg_runtime_b[num12] = 0L;
				avg_runtime_l[num12] = 0L;
				max_ipc_l[num12] = 0L;
				max_ipc_b[num12] = 0L;
				temp4[num12] = 0L;
				temp5[num12] = 0L;
			...
			topKvalue = 3;
			if (SchedulerRuntime > 29)
			{
				sysinfo.IsModelSaved = true;
				transformerScheduler.SetLearningEnabled(enabled: false);
				transformerScheduler.SetTopK(topKvalue);
			}
			else
			{
				Show("服务准备", "第一次运行服务需要30分钟时间训练调度神经网络，途中可能产生卡顿及未响应，此时间段挂机为最佳，当前系统空闲则可点击确认以继续", 0, 180);
				transformerScheduler._startTick = DateTime.Now.Ticks;
			}
			...
			void thread()
			{
				...
				traceEventSession.Source.Kernel.ThreadCSwitch += delegate(CSwitchTraceData data)
				{
					...
					int num17 = 0;
					int num18 = 0;
					num16 = data.OldThreadID;
					_ = data.NewThreadID;
					num17 = data.OldProcessID;
					num18 = data.NewProcessID;
					num13 = data.ProcessorNumber;
					currentprocessor[num13] = num13;
					UIntPtr threadAffinityMask4 = (UIntPtr)(ulong)Math.Pow(2.0, num13);
					currentprocnum_index = num13 / 2;
					datetime_new[num13] = DateTime.Now.Ticks;
					datetime_elapsed[num13] = (datetime_new[num13] - datetime_old[num13]) / 10;
					datetime_old[num13] = datetime_new[num13];
					long num19 = Math.Abs(num16 % 10000);
					long num20 = Math.Abs(num17 % 10000);
					if (num17 != 0)
					{
						coreinfo[num13].Threadcount++;
						coreinfo[num13].RunTime4queque += datetime_elapsed[num13];
						coreinfo[num13].RunTime4queque4sched += datetime_elapsed[num13];
						coreinfo[num13].RunTime4usage += datetime_elapsed[num13];
						coreinfo[num13].threadexecinfo.AddOrUpdate(num16, datetime_elapsed[num13]);
						coreinfo[num13].numberProcessor.AddData(datetime_elapsed[num13]);
						if ((int)((uint)(1 << num13) & affinitymask_little) > 0)
						{
							coreinfo[num13].accRuntimePerQ += datetime_elapsed[num13];
							if (coreinfo[num13].threadContrib.ContainsKey(num16))
							{
								coreinfo[num13].accRewardPerQ -= coreinfo[num13].threadContrib[num16];
							}
							long num21 = coreinfo[num13].accRuntimePerQ * data.OldThreadPriority;
							coreinfo[num13].accRewardPerQ += num21;
							coreinfo[num13].threadContrib[num16] = num21;
						}
					}
					if (num17 != 0 && num18 == 0)
					{
						coreinfo[num13].SustainedThreadcount++;
						coreinfo[num13].SustainedThreadcount4sched++;
						coreinfo[num13].Cycle++;
						coreinfo[num13].Accthreadcount += coreinfo[num13].threadexecinfo.Count;
						coreinfo[num13].threadexecinfo.Clear();
						coreinfo[num13].accRunTime4usage += coreinfo[num13].RunTime4usage;
						coreinfo[num13].Threadcount = 0L;
						coreinfo[num13].RunTime4usage = 0L;
						if ((int)((uint)(1 << num13) & affinitymask_little) > 0)
						{
							sysinfo.accRewordPerS += ((coreinfo[num13].threadContrib.Count > 0) ? (coreinfo[num13].accRewardPerQ / coreinfo[num13].threadContrib.Count) : 0);
							sysinfo.accQcount++;
							coreinfo[num13].accRuntimePerQ = 0L;
							coreinfo[num13].accRewardPerQ = 0L;
							coreinfo[num13].threadContrib.Clear();
						}
					}
					long num22 = DateTime.Now.Ticks - coreinfo[num13].DateTime;
					_ = DateTime.Now.Ticks;
					_ = coreinfo[num13].DateTime4sched;
					if (num22 > 200000 && coreinfo[num13].SustainedThreadcount > 10)
					{
						coreinfo[num13].Utilization4q = coreinfo[num13].RunTime4queque / coreinfo[num13].SustainedThreadcount;
						coreinfo[num13].Avgthreadcount = 100 * coreinfo[num13].Accthreadcount / coreinfo[num13].SustainedThreadcount;
						coreinfo[num13].QueueInterval = num22 / (coreinfo[num13].SustainedThreadcount * 10);
						myOls.RdmsrTx(197u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].instructions4sys_l = num15 | num14;
						coreinfo[num13].instructions4sys = Math.Max(coreinfo[num13].instructions4sys_l - coreinfo[num13].instructions4sys_e, 0L);
						coreinfo[num13].instructions4sys_e = coreinfo[num13].instructions4sys_l;
						myOls.RdmsrTx(198u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].cycles4sys_l = num15 | num14;
						coreinfo[num13].cycles4sys = Math.Max(coreinfo[num13].cycles4sys_l - coreinfo[num13].cycles4sys_e, 0L);
						sysinfo.total_runtime += coreinfo[num13].cycles4sys;
						coreinfo[num13].cycles4sys_e = coreinfo[num13].cycles4sys_l;
						myOls.RdmsrTx(194u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].missrate4c_l = num15 | num14;
						coreinfo[num13].missrate4c = Math.Max(coreinfo[num13].missrate4c_l - coreinfo[num13].missrate4c_e, 0L);
						coreinfo[num13].missrate4c_e = coreinfo[num13].missrate4c_l;
						myOls.RdmsrTx(195u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].load_l = num15 | num14;
						coreinfo[num13].load = Math.Max(coreinfo[num13].load_l - coreinfo[num13].load_e, 0L);
						coreinfo[num13].load_e = coreinfo[num13].load_l;
						myOls.RdmsrTx(196u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].store_l = num15 | num14;
						coreinfo[num13].store = Math.Max(coreinfo[num13].store_l - coreinfo[num13].store_e, 0L);
						coreinfo[num13].store_e = coreinfo[num13].store_l;
						myOls.RdmsrTx(193u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].mem_ordering_count_l = num15 | num14;
						coreinfo[num13].mem_ordering_count = Math.Max(coreinfo[num13].mem_ordering_count_l - coreinfo[num13].mem_ordering_count_e, 0L);
						coreinfo[num13].mem_ordering_count_e = coreinfo[num13].mem_ordering_count_l;
						coreinfo[num13].accRunTime4usage = 0L;
						coreinfo[num13].RunTime4usage = 0L;
						coreinfo[num13].ipc4c = coreinfo[num13].CalcRatio1(coreinfo[num13].instructions4sys, coreinfo[num13].cycles4sys, coreinfo[num13].ipc4c);
						coreinfo[num13].perf4c = coreinfo[num13].CalcRatio1(coreinfo[num13].instructions4sys, coreinfo[num13].RunTime4queque, coreinfo[num13].perf4c);
						coreinfo[num13].Utilization = coreinfo[num13].CalcRatio1(coreinfo[num13].instructions4sys, coreinfo[num13].SustainedThreadcount, coreinfo[num13].Utilization);
						if ((int)((uint)(1 << num13) & (affinitymask_big_phyx | affinitymask_big_smt)) > 0)
						{
							coreinfo[num13].missrateratio4c1 = coreinfo[num13].CalcRatio1(coreinfo[num13].missrate4c, coreinfo[num13].instructions4sys, coreinfo[num13].missrateratio4c1);
							sysinfo.total_instructions += coreinfo[num13].instructions4sys;
							coreinfo[num13].issue_width = (int)((coreinfo[num13].cycles4sys > 0) ? (coreinfo[num13].load / coreinfo[num13].cycles4sys) : coreinfo[num13].issue_width);
							if (coreinfo[num13].load - coreinfo[num13].store > 0)
							{
								coreinfo[num13].missrateratio4c = coreinfo[num13].CalcRatio1(coreinfo[num13].load - coreinfo[num13].store, coreinfo[num13].issue_width * coreinfo[num13].instructions4sys, coreinfo[num13].missrateratio4c);
								sysinfo.total_llcmiss += coreinfo[num13].load - coreinfo[num13].store;
								sysinfo.total_slots += coreinfo[num13].issue_width * coreinfo[num13].instructions4sys;
							}
						}
						else
						{
							coreinfo[num13].missrateratio4c1 = coreinfo[num13].CalcRatio1(coreinfo[num13].missrate4c, coreinfo[num13].instructions4sys, coreinfo[num13].missrateratio4c1);
							sysinfo.total_instructions += coreinfo[num13].instructions4sys;
							coreinfo[num13].issue_width = (int)((coreinfo[num13].cycles4sys > 0) ? ((coreinfo[num13].mem_ordering_count + coreinfo[num13].missrate4c + coreinfo[num13].load + coreinfo[num13].store) / coreinfo[num13].cycles4sys) : coreinfo[num13].issue_width);
							coreinfo[num13].missrateratio4c = coreinfo[num13].CalcRatio1(coreinfo[num13].mem_ordering_count + coreinfo[num13].missrate4c + coreinfo[num13].load, coreinfo[num13].issue_width * coreinfo[num13].instructions4sys, coreinfo[num13].missrateratio4c);
							sysinfo.total_llcmiss += coreinfo[num13].mem_ordering_count + coreinfo[num13].missrate4c + coreinfo[num13].load;
							sysinfo.total_slots += coreinfo[num13].issue_width * coreinfo[num13].instructions4sys;
						}
						coreinfo[num13].missrateratio4c2 = coreinfo[num13].CalcRatio1(coreinfo[num13].load, coreinfo[num13].instructions4sys, coreinfo[num13].missrateratio4c2);
						coreinfo[num13].mem_ordering = coreinfo[num13].CalcRatio1(coreinfo[num13].store, coreinfo[num13].instructions4sys, coreinfo[num13].mem_ordering);
						if (!sysinfo.IsModelSaved)
						{
							if ((int)((uint)(1 << num13) & (affinitymask_big_phyx | affinitymask_big_smt)) > 0)
							{
								UIntPtr threadAffinityMask5 = (UIntPtr)(ulong)(1 << num13);
								uint num23 = 0u;
								uint num24 = 0u;
							...
							{
								count4level3 = 0L;
								coreinfo[num13].P_state = 2;
								UIntPtr threadAffinityMask6 = (UIntPtr)(ulong)(1 << num13);
								uint num25 = 0u;
								uint num26 = 0u;
								myOls.RdmsrTx(1908u, ref num25, ref num26, threadAffinityMask6);
								myOls.WrmsrTx(1908u, (num25 & 0xFFFFFF) | 0x80000000u, num26, threadAffinityMask6);
								perflevel02++;
							}
						}
						else if ((int)((uint)(1 << num13) & (affinitymask_big_phyx | affinitymask_big_smt)) > 0)
						{
							if (coreinfo[num13].Utilization4q > 1000)
							{
								UIntPtr threadAffinityMask7 = (UIntPtr)(ulong)(1 << num13);
								uint num27 = 0u;
								uint num28 = 0u;
								myOls.RdmsrTx(1908u, ref num27, ref num28, threadAffinityMask7);
								myOls.WrmsrTx(1908u, (num27 & 0xFFFFFF) | 0x55000000, num28, threadAffinityMask7);
								perflevel2++;
							}
							else
							{
								UIntPtr threadAffinityMask8 = (UIntPtr)(ulong)(1 << num13);
								uint num29 = 0u;
								uint num30 = 0u;
								myOls.RdmsrTx(1908u, ref num29, ref num30, threadAffinityMask8);
								myOls.WrmsrTx(1908u, (num29 & 0xFFFFFF) | 0x80000000u, num30, threadAffinityMask8);
								perflevel3++;
							}
						}
						else
						{
							coreinfo[num13].P_state = 1;
							UIntPtr threadAffinityMask9 = (UIntPtr)(ulong)(1 << num13);
							uint num31 = 0u;
							uint num32 = 0u;
							myOls.RdmsrTx(1908u, ref num31, ref num32, threadAffinityMask9);
							myOls.WrmsrTx(1908u, (num31 & 0xFFFFFF) | 0x55000000, num32, threadAffinityMask9);
							perflevel01++;
						}
						coreFeatures[num13] = new float[9]
						{
							coreinfo[num13].Utilization,
							coreinfo[num13].Utilization4q,
							coreinfo[num13].Avgthreadcount,
							coreinfo[num13].QueueInterval,
							coreinfo[num13].missrateratio4c,
							coreinfo[num13].ipc4c,
							coreinfo[num13].perf4c,
							num13,
							coretype[num13]
						};
						coreinfo[num13].RunTime4queque = 0L;
						coreinfo[num13].SustainedThreadcount = 0L;
						coreinfo[num13].AccMaxTime = 0L;
						coreinfo[num13].Accthreadcount = 0L;
						coreinfo[num13].numberProcessor.Clear();
						coreinfo[num13].DateTime = DateTime.Now.Ticks;
					}
					if (currentthread != num17 && num17 != 0)
					{
						lock (lockProcessCreation)
						{
							findprocessinfo[num13] = FindProcess(ref processinfo[num20], num17);
							if (findprocessinfo[num13] == null)
							{
								findprocessinfo[num13] = UpdateProcessInfo(500, ref processinfo[num20], new ProcessInfo(num17, DateTime.Now.Ticks, 0L, 0L, 0L, 0L, 0L, 0L, 0, 0, 0L, 0L, 0L, 0, 0, DateTime.Now.Ticks, DateTime.Now.Ticks));
								findprocessinfo[num13].datetime_elapse = DateTime.Now.Ticks;
							}
						}
						findthreadinfo[num13] = FindThread(ref threadinfo[num19], num16);
						cnt_findnode++;
						if (findthreadinfo[num13] == null)
						{
							cnt_not_findnode++;
							findthreadinfo[num13] = UpdateThreadInfo(500, ref threadinfo[num19], new ThreadInfo(num16, DateTime.Now.Ticks, 0L, 0L, 0L, 0L, 0L, 50000L, 0L, 0L, 0L, 1, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0L, 0u, 0, new ThreadInfoSimp(num16, 0L, 0L, 0L, -1, 0, null), 0L, 0L, 0L, 0, 0L, 0L, 1, 0L, 0L, 0L, 0L, 0L, 0L, DateTime.Now.Ticks, DateTime.Now.Ticks, (IntPtr)(affinitymask_big_phyx | affinitymask_big_smt | affinitymask_little), random.Next(maxLP)));
							findthreadinfo[num13].Processinfo = findprocessinfo[num13];
							ThreadInfoSimp threadInfo = new ThreadInfoSimp(findthreadinfo[num13].Tid, 0L, 0L, 0L, 1, findthreadinfo[num13].Group, findthreadinfo[num13]);
							if (findprocessinfo[num13] != null)
							{
								ThreadInfoSimp head = findprocessinfo[num13].ThreadSet;
								findprocessinfo[num13].ThreadSet = UpdateThreadInfoSimp(500, ref head, threadInfo);
							}
							findthreadinfo[num13].Groupinfo = Perfgroup[perfstatenum - 1];
							findthreadinfo[num13].Perflvl = perfstatenum - 1;
							findthreadinfo[num13].Efflvl = 3;
							findthreadinfo[num13].demoteacc = DateTime.Now.Ticks;
							findthreadinfo[num13].SchedType = 1L;
							findthreadinfo[num13].Sched = 1;
						}
						findthreadinfo[num13].Count_sample++;
						findthreadinfo[num13].RunTime += datetime_elapsed[num13];
						findthreadinfo[num13].WaitTime += oldthread_waittime[num13];
						findthreadinfo[num13].Count_sample1++;
						findthreadinfo[num13].Duration -= datetime_elapsed[num13];
						if (findthreadinfo[num13].Duration < 0)
						{
							findthreadinfo[num13].Duration = 0L;
						}
						if (findthreadinfo[num13].Duration == 0L && findthreadinfo[num13].RunTime > 300000)
						{
							findthreadinfo[num13].Ipc_reset_count = findthreadinfo[num13].RunTime / findthreadinfo[num13].Count_sample;
							findthreadinfo[num13].Count_sample = 0L;
							findthreadinfo[num13].RunTime = 0L;
							findthreadinfo[num13].DateTime = DateTime.Now.Ticks;
						}
					}
					if ((sysinfo.Counter_sys_enabled != 0 || coreinfo[num13].CounterEnabled != 0) && coreinfo[num13].CounterEnabled == 1)
					{
						myOls.RdmsrTx(198u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register6_l = num15 | num14;
						myOls.RdmsrTx(195u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register3_l = num15 | num14;
						myOls.RdmsrTx(196u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register4_l = num15 | num14;
						myOls.RdmsrTx(194u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register2_l = num15 | num14;
						myOls.RdmsrTx(197u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register5_l = num15 | num14;
						myOls.RdmsrTx(193u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register1_l = num15 | num14;
						coreinfo[num13].Register6 = Math.Max(coreinfo[num13].Register6_l - coreinfo[num13].Register6_e, 0L);
						coreinfo[num13].Register5 = Math.Max(coreinfo[num13].Register5_l - coreinfo[num13].Register5_e, 0L);
						coreinfo[num13].Register4 = Math.Max(coreinfo[num13].Register4_l - coreinfo[num13].Register4_e, 0L);
						coreinfo[num13].Register3 = Math.Max(coreinfo[num13].Register3_l - coreinfo[num13].Register3_e, 0L);
						coreinfo[num13].Register2 = Math.Max(coreinfo[num13].Register2_l - coreinfo[num13].Register2_e, 0L);
						coreinfo[num13].Register1 = Math.Max(coreinfo[num13].Register1_l - coreinfo[num13].Register1_e, 0L);
						if (currentthread != num17 && num17 != 0)
						{
							findthreadinfo[num13].L4_miss += coreinfo[num13].Register3;
							findthreadinfo[num13].Ins_retire += coreinfo[num13].Register5;
							findthreadinfo[num13].PriorityAcc++;
							findthreadinfo[num13].avgruntime_total += datetime_elapsed[num13];
							if ((int)((uint)(1 << num13) & (affinitymask_big_phyx | affinitymask_big_smt)) > 0)
							{
								if (coreinfo[num13].Register3 - coreinfo[num13].Register4 > 0)
								{
									findthreadinfo[num13].L3_miss += coreinfo[num13].Register3 - coreinfo[num13].Register4;
									findthreadinfo[num13].CodeFootPrint_counter1 += coreinfo[num13].Register5 * coreinfo[num13].issue_width;
								}
								findthreadinfo[num13].L3_miss_1 += coreinfo[num13].Register2;
								findthreadinfo[num13].L3_miss_2 += coreinfo[num13].Register3;
								findthreadinfo[num13].L1_miss1 += coreinfo[num13].Register4;
								findthreadinfo[num13].L3_miss1 += coreinfo[num13].Register5;
							}
							if ((int)((uint)(1 << num13) & affinitymask_little) > 0)
							{
								findthreadinfo[num13].L3_miss += coreinfo[num13].Register1 + coreinfo[num13].Register2 + coreinfo[num13].Register3;
								findthreadinfo[num13].L3_miss_1 += coreinfo[num13].Register2;
								findthreadinfo[num13].L3_miss_2 += coreinfo[num13].Register3;
								findthreadinfo[num13].CodeFootPrint_counter1 += coreinfo[num13].Register5 * coreinfo[num13].issue_width;
								findthreadinfo[num13].L1_miss1 += coreinfo[num13].Register4;
							...
							findthreadinfo[num13].IntVal = (DateTime.Now.Ticks - findthreadinfo[num13].DateTime4interval) / 10;
							findthreadinfo[num13].L2_miss += coreinfo[num13].Register6;
							try
							{
								IntPtr intPtr2 = OpenThread((ThreadAccess)96u, bInheritHandle: false, (uint)num16);
								if (intPtr2 != IntPtr.Zero)
								{
									thread_priority[num13] = GetThreadPriority(intPtr2);
									CloseHandle(intPtr2);
								}
								else
								{
									thread_priority[num13] = 0;
								}
							}
							catch
							{
							...
							try
							{
								IntPtr intPtr3 = OpenProcess((ProcessAccess)1536u, bInheritHandle: false, (uint)num17);
								if (intPtr3 != IntPtr.Zero)
								{
									process_priority[num13] = GetPriorityClass(intPtr3);
									CloseHandle(intPtr3);
								}
								else
								{
									process_priority[num13] = 0;
								}
							}
							catch
							{
							...
							findthreadinfo[num13].Clock_big += ThreadPriorityMapper.GetFinalPriority(process_priority[num13], thread_priority[num13]);
							if (findthreadinfo[num13].PriorityAcc > 1000 || findthreadinfo[num13].Ins_retire > 300000)
							{
								findthreadinfo[num13].Ins_per_count = findthreadinfo[num13].CalcRatio(findthreadinfo[num13].Ins_retire, findthreadinfo[num13].PriorityAcc, findthreadinfo[num13].Ins_per_count);
								findthreadinfo[num13].UserModeRatio = (float)findthreadinfo[num13].L4_miss / (float)findthreadinfo[num13].Ins_retire;
								findthreadinfo[num13].Ins_big1 = findthreadinfo[num13].CalcRatio1(findthreadinfo[num13].L1_miss1, findthreadinfo[num13].L3_miss1, findthreadinfo[num13].Ins_big1);
								findthreadinfo[num13].CodeFootPrint = findthreadinfo[num13].CalcRatio(1000000 * findthreadinfo[num13].CodeFootPrint_counter1, findthreadinfo[num13].Ins_retire, findthreadinfo[num13].CodeFootPrint);
								findthreadinfo[num13].Ins_big = findthreadinfo[num13].CalcRatio1(findthreadinfo[num13].L3_miss, findthreadinfo[num13].CodeFootPrint_counter1, findthreadinfo[num13].Ins_big);
								findthreadinfo[num13].Ins_big2 = findthreadinfo[num13].CalcRatio1(findthreadinfo[num13].L3_miss_1, findthreadinfo[num13].CodeFootPrint_counter1, findthreadinfo[num13].Ins_big2);
								findthreadinfo[num13].Ins_big3 = findthreadinfo[num13].CalcRatio1(findthreadinfo[num13].L3_miss_2, findthreadinfo[num13].CodeFootPrint_counter1, findthreadinfo[num13].Ins_big3);
								findthreadinfo[num13].Ipc = findthreadinfo[num13].CalcRatio(findthreadinfo[num13].IntVal, findthreadinfo[num13].Count_sample1, findthreadinfo[num13].Ipc);
								findthreadinfo[num13].Clock = findthreadinfo[num13].CalcRatio1(findthreadinfo[num13].Ins_retire, findthreadinfo[num13].L2_miss, findthreadinfo[num13].Clock);
								findthreadinfo[num13].InsPressure = findthreadinfo[num13].CalcRatio1(findthreadinfo[num13].Clock_big, findthreadinfo[num13].PriorityAcc, findthreadinfo[num13].InsPressure);
								findthreadinfo[num13].avgruntime = findthreadinfo[num13].CalcRatio1(findthreadinfo[num13].WaitTime, findthreadinfo[num13].Count_sample1, findthreadinfo[num13].avgruntime);
								float[] array8 = new float[8]
								{
									findthreadinfo[num13].Ins_per_count,
									(float)findthreadinfo[num13].InsPressure,
									(float)findthreadinfo[num13].avgruntime,
									findthreadinfo[num13].Ipc,
									(float)findthreadinfo[num13].Clock,
									(float)findthreadinfo[num13].Ins_big,
									num13,
									coretype[num13]
								};
								findthreadinfo[num13].Processinfo.analyzer.AddOrUpdateThreadData(num16, findthreadinfo[num13].Ins_per_count);
								long ticks2 = DateTime.Now.Ticks;
								if (ticks2 - findthreadinfo[num13].Processinfo.Activethreadstat_datetime > 600000000 && findthreadinfo[num13].Processinfo.Activethreadstat_lock == 0)
								{
									findthreadinfo[num13].Processinfo.Activethreadstat_lock = 1;
									findthreadinfo[num13].Processinfo.analyzer.CleanInactiveThreads();
									findthreadinfo[num13].Processinfo.Activethreadstat_datetime = ticks2;
									findthreadinfo[num13].Processinfo.Activethreadstat_lock = 0;
								}
								if (SchedulerRuntime <= 29)
								{
									ScheduleResult scheduleResult = transformerScheduler.Schedule(array8, coreFeatures, maxLP, num16, findthreadinfo[num13].PrevCoreType, num13);
									sysinfo.threadMigrations += (((int)((uint)(1 << scheduleResult.BestCoreIndex) & affinitymask_little) > 0) ? 1 : 2) * findthreadinfo[num13].Ins_per_count;
									sysinfo.threadMigrations_count++;
									findthreadinfo[num13].PrevCoreType = scheduleResult.BestCoreIndex;
									findthreadinfo[num13].PrevCoreAffinity = scheduleResult.AffinityMask;
								}
								else
								{
									ScheduleResult scheduleResult2 = transformerScheduler.Schedule(array8, coreFeatures, maxLP, num16, findthreadinfo[num13].PrevCoreType, num13);
									sysinfo.threadMigrations += (((int)((uint)(1 << scheduleResult2.BestCoreIndex) & affinitymask_little) > 0) ? 1 : 2) * findthreadinfo[num13].Ins_per_count;
									sysinfo.threadMigrations_count++;
									findthreadinfo[num13].PrevCoreType = scheduleResult2.BestCoreIndex;
									findthreadinfo[num13].PrevCoreAffinity = scheduleResult2.AffinityMask;
									queue.Add(num16, findthreadinfo[num13].PrevCoreType, findthreadinfo[num13].PrevCoreAffinity, 100);
								}
								if (((uint)(1 << findthreadinfo[num13].PrevCoreType) & (affinitymask_big_phyx | affinitymask_big_smt)) == 0)
								{
									findthreadinfo[num13].CoreType = 0;
								}
								else
								{
									findthreadinfo[num13].CoreType = 1;
								}
								findthreadinfo[num13].Ins_retire = 0L;
								findthreadinfo[num13].PriorityAcc = 0L;
								findthreadinfo[num13].L3_miss1 = 0L;
								findthreadinfo[num13].L3_miss = 0L;
								findthreadinfo[num13].L1_miss1 = 0L;
								findthreadinfo[num13].L2_miss = 0L;
								findthreadinfo[num13].L3_miss_1 = 0L;
								findthreadinfo[num13].L3_miss_2 = 0L;
								findthreadinfo[num13].WaitTime = 0L;
								findthreadinfo[num13].CodeFootPrint_counter1 = 0L;
								findthreadinfo[num13].Count_sample1 = 0L;
								findthreadinfo[num13].Clock_big = 0L;
								findthreadinfo[num13].avgruntime_total = 0L;
								findthreadinfo[num13].DateTime4interval = DateTime.Now.Ticks;
							}
						}
					}
					if (currentthread != num17 && num17 != 0)
					{
						float num33 = Math.Max(GetFactor(findthreadinfo[num13].Clock_litte), 0.5f);
						int num34 = currentefflvl;
						int num35 = currentperflvl;
						_ = currentsmtlvl;
						int num36 = 0;
						switch (findthreadinfo[num13].CoreType)
						{
						case 0:
							if (findthreadinfo[num13].Duration == 0L)
							{
								if (num34 >= findthreadinfo[num13].Efflvl)
								{
									findthreadinfo[num13].Efflvl = num34;
								}
								else
								{
									findthreadinfo[num13].Efflvl = ((findthreadinfo[num13].Efflvl > 0) ? (findthreadinfo[num13].Efflvl - 1) : 0);
								}
								findthreadinfo[num13].Efflvl = ((findthreadinfo[num13].Efflvl < level_nodes_l[0] - 1) ? ((int)(level_nodes_l[0] - 1)) : findthreadinfo[num13].Efflvl);
								if (findthreadinfo[num13].Efflvl > (int)(level_nodes_l[0] - 1))
								{
									L2Bresidency++;
								}
								else
								...
								findthreadinfo[num13].Groupinfo = Effgroup[findthreadinfo[num13].Efflvl];
								findthreadinfo[num13].Sched = 1;
								findthreadinfo[num13].Duration = 100000L;
								findthreadinfo[num13].CoreType = 0;
							}
							break;
						case 1:
							...
							if (findthreadinfo[num13].Duration == 0L || num35 > findthreadinfo[num13].Perflvl)
							{
								if (num35 >= findthreadinfo[num13].Perflvl)
								{
									findthreadinfo[num13].Perflvl = num35;
								}
								else
								{
									findthreadinfo[num13].Perflvl = ((findthreadinfo[num13].Perflvl > 0) ? (findthreadinfo[num13].Perflvl - 1) : 0);
								}
								findthreadinfo[num13].Perflvl = (int)GetLevel(1, findthreadinfo[num13].Perflvl);
								findthreadinfo[num13].Groupinfo = Perfgroup[findthreadinfo[num13].Perflvl];
								findthreadinfo[num13].Sched = 1;
								findthreadinfo[num13].Duration = 100000L;
								findthreadinfo[num13].CoreType = 1;
							}
						...
						if (DateTime.Now.Ticks - queue.datetimeticks > 100000)
						{
							queue.datetimeticks = DateTime.Now.Ticks;
							queue.CanSchedule = true;
						}
						if (SchedulerRuntime > 29)
						{
							if (queue.CanSchedule)
							{
								queue.CanSchedule = false;
								ThreadScheduleQueue.ScheduleItem first = queue.GetFirst();
								ThreadInfo threadInfo2 = FindThread(ref threadinfo[Math.Abs(first.ThreadId % 10000)], first.ThreadId);
								if (first != null)
								{
									try
									{
										IntPtr intPtr4 = OpenThread((ThreadAccess)96u, bInheritHandle: false, (uint)first.ThreadId);
										if (intPtr4 != IntPtr.Zero)
										{
											ProcessBehaviorAnalyzer.ThreadCoreResult threadCoreType = threadInfo2.Processinfo.analyzer.GetThreadCoreType(first.ThreadId);
											...
											if (threadCoreType.CoreType == 1)
											{
												...
												SetThreadIdealProcessor(intPtr4, first.ScheduleCore);
											}
											else if ((int)((uint)(1 << first.ScheduleCore) & affinitymask_little) > 0)
											{
						...
						else if (findthreadinfo[num13].Sched == 1)
						{
							try
							{
								IntPtr intPtr5 = OpenThread((ThreadAccess)96u, bInheritHandle: false, (uint)num16);
								if (intPtr5 != IntPtr.Zero)
								{
									SetThreadAffinityMask(intPtr5, (IntPtr)(1 << findthreadinfo[num13].PrevCoreType));
									SetThreadIdealProcessor(intPtr5, findthreadinfo[num13].PrevCoreType);
									CloseHandle(intPtr5);
								}
							}
							catch
							...
							findthreadinfo[num13].Sched = 0;
						}
					}
					if (sysinfo.Counter_sys_enabled == 1 || coreinfo[num13].CounterEnabled == 1)
					{
						myOls.RdmsrTx(198u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register6_l = num15 | num14;
						coreinfo[num13].Register6_e = coreinfo[num13].Register6_l;
						myOls.RdmsrTx(195u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register3_l = num15 | num14;
						coreinfo[num13].Register3_e = coreinfo[num13].Register3_l;
						myOls.RdmsrTx(196u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register4_l = num15 | num14;
						coreinfo[num13].Register4_e = coreinfo[num13].Register4_l;
						myOls.RdmsrTx(194u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register2_l = num15 | num14;
						coreinfo[num13].Register2_e = coreinfo[num13].Register2_l;
						myOls.RdmsrTx(197u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register5_l = num15 | num14;
						coreinfo[num13].Register5_e = coreinfo[num13].Register5_l;
						myOls.RdmsrTx(193u, ref num14, ref num15, threadAffinityMask4);
						coreinfo[num13].Register1_l = num15 | num14;
						coreinfo[num13].Register1_e = coreinfo[num13].Register1_l;
					}
					if (sysinfo.Counter_sys_enabled == 1 && coreinfo[num13].CounterEnabled == 0)
					{
						coreinfo[num13].CounterEnabled = 1;
					}
					else if (sysinfo.Counter_sys_enabled == 0 && coreinfo[num13].CounterEnabled == 1)
					{
						coreinfo[num13].CounterEnabled = 0;
					}
					oldthread_waittime[num13] = data.NewThreadWaitTime;
					if ((int)((uint)(1 << num13) & affinitymask_big_phyx) > 0 || (int)((uint)(1 << num13) & affinitymask_big_smt) > 0)
					{
						big_actual++;
					}
					else
					...
					if (DateTime.Now.Ticks - sysinfo.Datetime4corecount > 200000 && sysinfo.updateCoreCount)
					{
						sysinfo.updateCoreCount = false;
						var (num37, num38) = CalcCoreActiveThreadSum(affinitymask_big_phyx | affinitymask_big_smt, affinitymask_little, coreFeatures);
						sysinfo.activeThread_big += (long)num37;
						sysinfo.activeThread_little += (long)num38;
						sysinfo.accIdleTime += CalcCoreDimDifferenceMin(coreFeatures);
						sysinfo.accIdleCount++;
						sysinfo.Datetime4corecount = DateTime.Now.Ticks;
						sysinfo.updateCoreCount = true;
					}
					if (DateTime.Now.Ticks - sysinfo.Datetime > 10000000 && sysinfo.update)
					{
						sysinfo.update = false;
						myOls.RdmsrTx(1553u, ref num14, ref num15, threadAffinityMask4);
						sysinfo.total_energy_l = num15 | num14;
						...
						sysinfo.little_big_ratio = ((sysinfo.activeThread_little + sysinfo.activeThread_big > 0) ? ((float)sysinfo.activeThread_little / (float)(sysinfo.activeThread_little + sysinfo.activeThread_big)) : sysinfo.little_big_ratio);
						sysinfo.accReword_ratio = ((sysinfo.accQcount > 0) ? ((float)sysinfo.accRewordPerS / (float)sysinfo.accQcount) : sysinfo.accReword_ratio);
						sysinfo.p_e_ratio = ((sysinfo.total_energy > 0) ? ((float)sysinfo.total_instructions / (float)sysinfo.total_energy) : sysinfo.p_e_ratio);
						sysinfo.BTB_miss_ratio = ((sysinfo.total_runtime > 0) ? ((float)sysinfo.total_instructions / (float)sysinfo.total_runtime) : sysinfo.BTB_miss_ratio);
						sysinfo.avgthreadMigrations = ((sysinfo.total_slots > 0) ? ((0f - (float)sysinfo.total_llcmiss) / (float)sysinfo.total_slots) : sysinfo.avgthreadMigrations);
						transformerScheduler.UpdateTAT(sysinfo.accReword_ratio, sysinfo.little_big_ratio, sysinfo.avgthreadMigrations, sysinfo.BTB_miss_ratio, little_num, big_total);
						sysinfo.accQcount = 0L;
						sysinfo.accRewordPerS = 0L;
						sysinfo.total_energy = 0L;
						sysinfo.total_slots = 0L;
						sysinfo.total_instructions = 0L;
						sysinfo.total_llcmiss = 0L;
						sysinfo.activeThread_little = 0L;
						sysinfo.activeThread_big = 0L;
						sysinfo.accIdleTime = 0f;
						sysinfo.accIdleCount = 0L;
						sysinfo.threadMigrations = 0f;
						sysinfo.threadMigrations_count = 0L;
		...
		private void OnTimedEvent(object sender, ElapsedEventArgs e)
		{
			...
			if (count_stat7 > 60)
			{
				count_stat7 = 0L;
				SchedulerRuntime = transformerScheduler.GetRuntime();
				if (SchedulerRuntime > 29 && !sysinfo.IsModelSaved)
				{
					...
					transformerScheduler.SetLearningEnabled(enabled: false);
					transformerScheduler.SetTopK(topKvalue);
					Show("服务准备完成", "调度神经网络训练已经完成，重启系统以应用更改", 0, 60, wait: false);
				}
			}
			count_stat++;
			if (count_stat > 10)
			{
				...
				string contents = "统计数据" + Environment.NewLine + "高负载差异:" + islarge + Environment.NewLine + "低负载差异:" + issmall + Environment.NewLine + "tempp:" + tempp + Environment.NewLine + "tempk:" + tempk + Environment.NewLine + "templ:" + templ + Environment.NewLine + "人工调度override:" + tempj + Environment.NewLine + "神经网络统计信息1:" + transformerScheduler.GetStatistics(maxLP) + Environment.NewLine + "神经网络统计信息2:" + transformerScheduler.GetLearningReport() + Environment.NewLine + "神经网络统计信息3:" + transformerScheduler.GetAttentionHeadReport(maxLP) + Environment.NewLine + "实际分配大核:" + big_actual + Environment.NewLine + "实际分配小核:" + little_actual + Environment.NewLine + "=============================" + Environment.NewLine + "大核性能状态:" + perflevel2 + Environment.NewLine + "大核能效状态:" + perflevel3 + Environment.NewLine + "=============================" + Environment.NewLine + "小核性能状态:" + perflevel01;
