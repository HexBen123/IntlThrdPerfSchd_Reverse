...
public partial class Service1 : ServiceBase
{
	...
	protected override void OnStart(string[] args)
	{
		...
		learner = new OnlineLearningManager(realtimeScheduler);
		learner.SetNumCores(maxLP);
		learner.EnableImmediateReward = true;
		learner.CurrentImmediateRewardMode = OnlineLearningManager.ImmediateRewardMode.CoreStateBased;
		learner.EnableEnergyReward = false;
			...
			session.Source.Kernel.ThreadCSwitch += delegate(CSwitchTraceData data)
			{
				...
				if (num19 > 300000 && coreinfo[num10].SustainedThreadcount > 10)
				{
					...
					coreFeatures[num10] = new float[7]
					{
						...
						((int)((uint)(1 << num10) & (affinitymask_big_phyx | affinitymask_big_smt)) > 0) ? (coreinfo[num10].perf4c * 10f) : coreinfo[num10].perf4c,
