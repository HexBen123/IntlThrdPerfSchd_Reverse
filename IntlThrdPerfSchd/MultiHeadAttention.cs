...
namespace IntlThrdPerfSchd
{
	public class MultiHeadAttention
	{
		...
		public void Backward(ReadOnlySpan<float> gradOutput, float learningRate = 0.001f)
		{
			...
			Wo.Backward(_gradConcatHeads.AsSpan(0, _dModel), learningRate, accumulateGrad: false);
			...
			Wq.Backward(_gradQProj, learningRate, accumulateGrad: false);
		}
		...
		public void SelfForward(ReadOnlySpan<float> input, Span<float> output)
		{
			...
			for (int j = 0; j < _nHead; j++)
			{
				...
				Span<float> span2 = _kProj.AsSpan(start, _dK);
				float num = VectorMathNew.DotProduct(span, span2) / (float)Math.Sqrt(_dK);
				_attentionScores[j * 64] = num;
				_attentionWeights[j * 64] = 1f;
				Span<float> span3 = _vProj.AsSpan(start, _dV);
				Span<float> span4 = _headOutputs.AsSpan(j * _dV, _dV);
		...
		public void SelfBackward(ReadOnlySpan<float> gradOutput, float learningRate = 0.001f)
		{
			...
			Wq.Backward(_gradQProj, learningRate, accumulateGrad: false);
			Wk.Backward(new ReadOnlySpan<float>(_gradKeys, 0, _dModel), learningRate, accumulateGrad: false);
			Wv.Backward(new ReadOnlySpan<float>(_gradValues, 0, _dModel), learningRate, accumulateGrad: false);
		}
