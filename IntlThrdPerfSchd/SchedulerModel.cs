...
public class TransformerScheduler
{
	...
	private void ComputeWindowStatistics()
	{
		...
		MathHelper.Clear(_threadFeatureMean.AsSpan(0, 5));
		MathHelper.Clear(_coreFeatureMean.AsSpan(0, 7));
		MathHelper.Clear(_threadFeatureStd.AsSpan(0, 5));
	...
	public int Schedule(ReadOnlySpan<float> threadFeatures, float[][] coreFeatures, int numCores, int threadId, int predictedCore, int actualCore)
	{
		...
		for (int j = 0; j < numCores; j++)
		{
			...
			{
				UpdateCoreEmbedding(coreFeat, j, normalizedCore, _coreEncoded[j].AsSpan());
		...
		Span<float> coreScoreSlice = _coreScore.AsSpan(0, numCores);
		attentionWeights.CopyTo(coreScoreSlice);
		float sumScores = VectorMathNew.Sum(coreScoreSlice);
		float maxScore;
		int selectedCore = VectorMathNew.FindMaxIndex(coreScoreSlice, out maxScore);
		if (sumScores > 0f)
		{
			MathHelper.Scale(coreScoreSlice, 1f / sumScores);
		}
	...
	private void UpdateTATInternal(float currentTAT)
	{
		...
		int actualDecisions = Math.Min(decisionsToLearn, 10000);
		for (int i = 0; i < actualDecisions; i++)
		{
			_learningWeights[i] = (float)Math.Exp(-0.5f * (float)(actualDecisions - 1 - i));
			totalWeight += _learningWeights[i];
		}
		float totalAdvantage = 0f;
		for (int j = 0; j < actualDecisions; j++)
		{
			int idx = startIdx + j;
			...
			float weightedReward = (record.Reward = reward * (_learningWeights[j] / totalWeight));
			...
			_threadEncoder.Backward(gradQuery, batchLearningRate);
			float[] gradEncoded = _threadEncoder.InputGradients;
			_threadEmbedding.Backward(gradEncoded, batchLearningRate);
			float[] gradCoreV = _crossAttention.GetValueGradients();
			int numCores = _crossAttention.GetCachedNumCores();
			float[] gradCoreEncoded = new float[64];
			for (int c = 0; c < numCores; c++)
			{
				for (int d = 0; d < 64; d++)
				{
					gradCoreEncoded[d] += gradCoreV[c * 64 + d];
				}
			}
			_coreEncoder.Backward(gradCoreEncoded.AsSpan(), batchLearningRate);
			float[] gradCoreEmbed = _coreEncoder.InputGradients;
			_coreEmbedding.Backward(gradCoreEmbed, batchLearningRate);
		...
		_threadEncoder.ApplyGradients(batchLearningRate);
		_coreEncoder.ApplyGradients(batchLearningRate);
		_stats.AvgLoss = _stats.AvgLoss * 0.9f + totalAdvantage / (float)Math.Max(actualDecisions, 1) * 0.1f;
		long currentTick = DateTime.Now.Ticks;
	...
	private void PerformBatchTraining()
	{
		...
		for (int s = 0; s < sampleSize; s++)
		{
			...
			_threadEmbedding.Backward(gradEncoded, batchLearningRate);
			float[] gradCoreV = _crossAttention.GetValueGradients();
			int numCoresBatch = _crossAttention.GetCachedNumCores();
			float[] gradCoreEncoded = new float[64];
			for (int c = 0; c < numCoresBatch; c++)
			{
				for (int d = 0; d < 64; d++)
				{
					gradCoreEncoded[d] += gradCoreV[c * 64 + d];
	...
	public string GetAttentionHeadReport(int numCores)
	{
		...
		{
			_reportMaxCores[j] = 0;
			_reportMaxWeights[j] = headWeights[j][0];
			for (int c = 1; c < numCores; c++)
			{
				if (headWeights[j][c] > _reportMaxWeights[j])
				{
					_reportMaxWeights[j] = headWeights[j][c];
					_reportMaxCores[j] = c;
				}
			}
		}
		for (int k = 0; k < numCores; k++)
		{
			...
			for (int l = 0; l < 8; l++)
			{
				string val = ((k == _reportMaxCores[l]) ? $"*{headWeights[l][k]:F4}" : $" {headWeights[l][k]:F4}");
				sb.Append($"{val,-8}│");
			}
		...
		for (int n = 0; n < 8; n++)
		{
			...
			sb.AppendLine($"  H{n}: Max→Core{_reportMaxCores[n]} ({_reportMaxWeights[n]:F4}), Avg={sum / (float)numCores:F4}");
		}
		return sb.ToString();
	}
