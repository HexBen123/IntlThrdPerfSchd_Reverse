	...
	public class TransformerScheduler
	{
		...
		public ScheduleResult Schedule(ReadOnlySpan<float> threadFeatures, float[][] coreFeatures, int numCores, int threadId, int predictedCore, int actualCore)
		{
			...
			for (int i = 0; i < numCores; i++)
			{
				...
				{
					throw new ArgumentOutOfRangeException("coreFeatures", $"coreFeatures[{i}].Length={coreFeatures[i]?.Length ?? -1} < CORE_INPUT_DIM={9}");
				}
			}
			...
			if (num5 < 10)
			{
				currentLearningRate = 0.01f;
			}
			else if (num5 < 17)
			{
				currentLearningRate = 0.003f;
			}
			else if (num5 < 24)
			{
		...
		private int CountParameters()
		{
			...
			num += _threadEncoder.Norm2.Gamma.Length + _threadEncoder.Norm2.Beta.Length;
			return num;
		}
		...
		public string GetLearningReport()
		{
			...
			stringBuilder.AppendLine($"Current value: {-_stats.LastNewMetric:F2}");
			stringBuilder.AppendLine($"Baseline value: {-_baselineNewMetric:F2}");
			stringBuilder.AppendLine($"Avg value: {-_stats.AvgNewMetric:F2}");
			stringBuilder.AppendLine($"Min value: {-_stats.MaxNewMetric:F2}");
			stringBuilder.AppendLine($"Max value: {-_stats.MinNewMetric:F2}");
			stringBuilder.AppendLine($"Value Trend: {-_stats.NewMetricTrend:F3} /step");
			stringBuilder.AppendLine();
